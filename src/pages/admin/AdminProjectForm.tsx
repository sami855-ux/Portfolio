...
export default function AdminProjectForm() {
  ...
  useEffect(() => {
    ...
    const loadProject = async () => {
      ...
      if (!targetProj && isSupabaseConfigured && !id.startsWith("demo")) {
        try {
  ...
  const handleGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadPromises = files.map((f) =>
        uploadToSupabaseStorage(f, "portfolio-images", "projects")
      )
  ...
  // Robust Supabase Save Helper
  const saveProjectToSupabase = async (projPayload: Project, isUpdate: boolean) => {
    ...
    if (isUpdate && projId && !projId.startsWith("demo")) {
      ...
      const { error } = await supabase.from("projects").update(projPayload).eq("id", projId)
      if (!error) return { success: true, id: projId }

      console.warn("Supabase array update notice:", error.message)
      ...
      const { error: err2 } = await supabase.from("projects").update(payloadJSON).eq("id", projId)
      ...
      const { error: err3 } = await supabase.from("projects").update(payloadComma).eq("id", projId)
      ...
      const { error: err4 } = await supabase.from("projects").update(payloadNoImages).eq("id", projId)
      if (err4) throw new Error(err4.message)
      return { success: true, id: projId }
    } else {
      // 1. Try insert as string array
      const { data, error } = await supabase.from("projects").insert([dataToSave]).select().single()
      if (!error && data) return { success: true, id: data.id }

      console.warn("Supabase array insert notice:", error?.message)
      ...
      const { data: d2, error: err2 } = await supabase.from("projects").insert([payloadJSON]).select().single()
      ...
      const { data: d3, error: err3 } = await supabase.from("projects").insert([payloadComma]).select().single()
      if (!err3 && d3) return { success: true, id: d3.id }

      // 4. Last fallback
      const { images, ...payloadNoImages } = dataToSave
      const { data: d4, error: err4 } = await supabase.from("projects").insert([payloadNoImages]).select().single()
      if (err4) throw new Error(err4.message)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let cleanImages = (project.images || []).map((img) => (typeof img === "string" ? img.trim() : img)).filter(Boolean)
      ...
      if (isSupabaseConfigured) {
        const savedRes = await saveProjectToSupabase(updatedProject, isEditing)
        if (savedRes?.id) {
