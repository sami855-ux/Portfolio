...
export default function AdminSkills() {
    ...
    try {
      if (isApiConfigured) {
        ...
        if (isUUID) {
          ...
          const { id: _id, ...newSkillData } = skillToSave
          const { data: inserted, error: insertErr } = await apiClient
            .from("skills")
            .insert([{
      ...
      toast.error("Failed to save skill: " + (err instanceof Error ? err.message : "Unknown error"), { id: toastId })
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleSeedDefaultSkills = async () => {
    ...
    try {
      if (isApiConfigured) {
        ...
        const { error } = await apiClient.from("skills").insert(payload)
        if (error) {
          console.warn("API insert error during seed:", error)
        }
      }
      ...
      toast.error("Failed to seed skills: " + (err instanceof Error ? err.message : "Unknown error"), { id: toastId })
    }
  }
  ...
  const confirmDeleteSkill = async () => {
    ...
    try {
      ...
      if (isApiConfigured && isUUID) {
        const { error } = await apiClient.from("skills").delete().eq("id", deleteDialog.id)
      ...
      setDeleteDialog((prev) => ({
        ...
        isLoading: false,
        isError: true,
        errorMessage: msg || "Failed to delete skill. Please try again.",
      }))
    }
  }
