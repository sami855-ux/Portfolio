...
export default function AdminResetPassword() {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 10) {
      const err = "Password must be at least 10 characters long."
      setErrorMsg(err)
      toast.error(err)
      return
    }
    ...
    const { error } = await apiClient.auth.updateUser({ password: newPassword, currentPassword })
    if (error) {
      ...
      >
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-4">
          <div>
            ...
            <Input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} required autoComplete="current-password" className="h-12 bg-[#181818] border-none text-white rounded-2xl px-4" />
          </div>
          <div>
