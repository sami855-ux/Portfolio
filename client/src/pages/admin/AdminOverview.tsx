...
export default function AdminOverview() {
  ...
  useEffect(() => {
    const loadOverviewMessages = async () => {
      if (isApiConfigured) {
        try {
          const { data: msgData, error: msgErr } = await apiClient
            .from("messages")
            .select("*")
            .order("created_at", { ascending: false })

          if (msgErr) {
            console.warn("API messages fetch error:", msgErr.message)
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[#202020] border-none p-6 rounded-3xl space-y-3">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              {isApiConfigured
                ? "Connected to the live Node.js API and PostgreSQL database."
            ...
            <div className="bg-[#181818] p-3.5 rounded-2xl border-none text-[11px] font-mono text-gray-300 space-y-1">
              <div className="text-green-500 font-bold"># API Configuration</div>
              <div>VITE_API_URL=https://api.example.com/api</div>
              <div>CORS_ORIGINS=https://portfolio.example.com</div>
            </div>
          </div>
        </div>
      </div>
