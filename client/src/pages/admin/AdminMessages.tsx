...
import { Trash2, Send, Inbox, Mail, CheckCircle2, Eye } from "lucide-react"
...
export default function AdminMessages() {
  ...
  const loadData = async () => {
    ...
    try {
      if (isApiConfigured) {
        const { data: msgData, error: fetchErr } = await apiClient
          .from("messages")
          .select("*")
        ...
        if (fetchErr) {
          ...
          if (fetchErr.message.includes("JWT issued at future") || fetchErr.message.includes("jwt")) {
            ...
            setErrorMsg("Could not connect to the API inbox: " + fetchErr.message)
          }
        } else if (msgData) {
      ...
      setErrorMsg("Failed to load inbox messages: " + (err instanceof Error ? err.message : "Unknown error"))
    } finally {
      setLoading(false)
    }
  }
  ...
  const toggleMessageRead = async (id?: string, currentRead?: boolean) => {
    ...
    try {
      if (isApiConfigured && !id.startsWith("demo")) {
        const { error: updateErr } = await apiClient.from("messages").update({ is_read: nextRead }).eq("id", id)
        if (updateErr) {
  ...
  const confirmDeleteMessage = async () => {
    ...
    try {
      if (isApiConfigured && !deleteDialog.id.startsWith("demo")) {
        const { error: delErr } = await apiClient.from("messages").delete().eq("id", deleteDialog.id)
