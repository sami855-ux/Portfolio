  ...
  const confirmDeleteLink = async () => {
    ...
    try {
      if (isApiConfigured && !deleteDialog.id.startsWith("demo")) {
        const { error } = await apiClient.from("contact_links").delete().eq("id", deleteDialog.id)
                ...
                try {
                  if (isApiConfigured) {
                    ...
                    if (isUUID) {
                      ...
                      const { data: inserted, error } = await apiClient
                        .from("floating_cards")
                        .insert([{
                          name: cardData.name,
        ...
        onConfirm={async () => {
          ...
          try {
            if (isApiConfigured && !deleteDialog.id.startsWith("demo")) {
              ...
              const { error } = await apiClient.from(table).delete().eq("id", deleteDialog.id)
              if (error) throw error
            }
